...
| 4 — Opt-in Rename Tool | `m4-rename` | #4 | ✅ Merged | Separate `RenameView` (off by default; library button). Pure `rename.rs`: `canonical_name`/`sanitize`, `build_plan` (ok/noop/conflict), crash-safe `execute` (manifest-line-before-rename + TOCTOU re-validate + per-item failure isolation, never overwrite/swap), tolerant idempotent `undo`. JSONL manifests in `<app_data>/rename-manifests/`. DB file_path/raw_filename kept in sync on apply+undo. Commands preview/apply/undo_renames. `rename` walkthrough (preview→apply→undo round-trip). 39 FE + 25 Rust tests (incl. `rename_roundtrip`). Screenshot-verified full round-trip (01-preview ≡ 03-undone). **Deferred to M5:** grouping-override UI writing `grouping_overrides` (split out per scope discipline — independent subsystem). |
| 5 — Grouping-Override Review UI | `m5-grouping` | — | 📝 Plan ready, awaiting execution | **Plan written (Opus):** `docs/superpowers/plans/2026-06-11-audioshelf-m5-grouping.md`. Reviewable/overridable grouping (spec §4): per-chapter Work/Ch# edit on Author Detail; merge/split/reassign via `grouping_overrides` + DB-only `regroup_author`, re-applied during scan, never written to disk. Was bundled into M4's row; separated because it's an independent subsystem. **Next session: execute this plan (Sonnet).** |
| 6 — Settings & Library-Root Picker | `m6-settings` | — | [ ] Not started | **Makes the app standalone-usable** (today the library root only comes from the `--library` launch flag — no in-app way to choose it). Spec §5 `settings` table + §12. In-app folder picker (Tauri dialog) → persist root in `settings` → scan → browse; change-root + re-scan; remember last root on launch. Plan with `writing-plans` first. |
| 7 — Scale & Search Polish | `m7-polish` | — | [ ] Not started | Finishes the v1 spec. Spec §4/§6/§13: virtualize the author list (300+ authors / 10k+ files target); extend search to works + chapters (today it filters authors only); generated cover placeholders (color + initials per author). Plan with `writing-plans` first. After M7 the v1 spec (design doc §1–§13) is fully implemented. |

**How to update:** after merging a milestone, flip its row to ✅ Merged, fill in the PR number, and add a
one-line note of what shipped plus any deferred minor issues. **This table is the single source of
truth for "what to build next"** — a fresh session picks the topmost row that is not yet ✅ Merged (a
`📝 Plan ready` row means execute the linked plan; a `[ ] Not started` row means write its plan first,
then execute). The design spec (`docs/superpowers/specs/2026-06-11-audioshelf-design.md` §1–§13) is the
authoritative scope; M7 completes it.
