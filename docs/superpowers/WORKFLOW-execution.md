...
| 5 — Grouping-Override Review UI | `m5-grouping` | — | [ ] Not started | Reviewable/overridable grouping (spec §4): merge/split works, reassign chapter, set base title / chapter number — all writing `grouping_overrides`, re-applied during scan, never written to disk. Was bundled into M4's row; separated because it's an independent subsystem. |
