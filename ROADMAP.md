...
| 5 | Grouping-Override Review UI | 📝 Plan ready | [M5](docs/superpowers/plans/2026-06-11-audioshelf-m5-grouping.md) | — | **Execute next (Sonnet).** Per-chapter Work/Ch# edit on Author Detail; DB-only `regroup_author` over `grouping_overrides`, re-applied on scan, never to disk. Merge/split/reassign. |
| 6 | Settings & Library-Root Picker | [ ] Not started | — | — | Makes the app standalone-usable (today root only comes from `--library` flag). Spec §5/§12: in-app folder picker → persist root in `settings` → scan → browse; change-root + re-scan. Plan first. |
