...
| # | Title | Status | Plan | PR | Notes |
|---|-------|--------|------|----|----|
| 8 | Embedded Cover Art | 📝 Plan ready | [M8](docs/superpowers/plans/2026-06-12-audioshelf-m8-cover-art.md) | — | **Execute next.** Read cover images from audio tags (`lofty` pictures) **or** a folder image (`cover/folder/front.*`); thumbnail via `image`; cache under `<app_data>/covers/` and serve through the existing asset protocol. On-the-fly `get_work_cover`/`get_author_cover` commands (NO schema change). `<Cover>` lazy-loads with client cache, falls back to the color+initials `<Swatch>`. Folder-image fixtures + `covers` walkthrough; `imagesSettled()` before cover screenshots. |
